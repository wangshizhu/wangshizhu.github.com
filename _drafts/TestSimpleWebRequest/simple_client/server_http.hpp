...
namespace SimpleWeb {
  ...
  protected:
    ...
    std::shared_ptr<Connection> create_connection(Args &&... args) noexcept {
      auto connections_tmp = this->connections;
      auto connections_mutex_tmp = this->connections_mutex;
      auto connection = std::shared_ptr<Connection>(new Connection(handler_runner, std::forward<Args>(args)...), [connections_tmp, connections_mutex_tmp](Connection *connection) {
        {
          std::unique_lock<std::mutex> lock(*connections_mutex_tmp);
          auto it = connections_tmp->find(connection);
          if(it != connections_tmp->end())
			  connections_tmp->erase(it);
        }
        delete connection;
      });
      {
        std::unique_lock<std::mutex> lock(*connections_mutex_tmp);
		connections_tmp->emplace(connection.get());
      }
      return connection;
    }
