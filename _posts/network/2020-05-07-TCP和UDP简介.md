...
这也导致TCP描述符对应的连接上发出一个FIN包

### CLOSE_WAIT
--------------------------------------------------

对于服务端程序如果实际生产过程中出现大量CLOSE_WAIT连接，这些CLOSE_WAIT连接保持了很长时间，说明服务端程序出现了**套接字泄露**，结合着上面的TCP四次挥手，服务端程序出现CLOSE_WAIT连接，
...
还有一个点需要验证，就是上面挥手步骤3提到的：**应用程序可以通过read调用来感知这个FIN包，这个EOF会被放在已排队等候的其他已接收的数据之后**。验证思路：
